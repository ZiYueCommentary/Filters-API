...
public class HotswapConfigLoader implements IdentifiableResourceReloadListener
{
    ...
    public CompletableFuture<Void> reload(Synchronizer synchronizer, ResourceManager manager, Profiler prepareProfiler, Profiler applyProfiler, Executor prepareExecutor, Executor applyExecutor) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            for (Resource resource : resources) {
                try (Reader reader = resource.getReader()) {
                    ...
                    for (Map.Entry<String, JsonElement> jsonTab : json.entrySet()) {
                        ...
                        final Optional<ItemGroup> creativeModeTab = ItemGroups.getGroups().stream().filter(group -> group.getId().equals(Identifier.tryParse(jsonTab.getKey()))).findAny();
            ...
            return null;
        }).thenCompose(synchronizer::whenPrepared).thenAcceptAsync((result) -> {}, applyExecutor);
    }
}
