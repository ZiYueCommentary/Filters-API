        ...
        for (Resource resource : resources) {
            try (Reader reader = resource.getReader()) {
                ...
                for (Map.Entry<String, JsonElement> jsonTab : json.entrySet()) {
                    ...
                    final Optional<ItemGroup> creativeModeTab = Registries.ITEM_GROUP.getOrEmpty(Identifier.of(jsonTab.getKey()));
                    ...
                    for (Map.Entry<String, JsonElement> filters : jsonTab.getValue().getAsJsonObject().entrySet()) {
                        ...
                        if (id.equals("uncategorized")) {
                            ...
                            final Identifier uncategorizedIcon = Identifier.of(config.has("icon") ?
                        ...
                        final Identifier icon = Identifier.of(config.get("icon").getAsString());
                        if (usedId.contains(id)) {
                            FiltersApi.LOGGER.warn("Duplicate filter {} in tab {}", id, jsonTab.getKey());
                            continue;
                        }
                        usedId.add(id);
                        list.add(new PendingFilter(id, title, icon, config.get("items").getAsJsonArray().asList().stream().map(item -> Identifier.of(item.getAsString())).toList()));
                    }
